...
class DatabaseOperations:
    def __init__(self):
        self.DB_FILE_PATH = os.path.join(os.path.dirname(__file__), '..', '..', 'database', 'market_mood.db')

    def insert_stock_prices(self, stock_prices):
        conn = sqlite3.connect(self.DB_FILE_PATH)
        c = conn.cursor()
        for price in stock_prices:
            c.execute('''SELECT COUNT(*) FROM stock_prices WHERE symbol = ? AND date = ?''', (price['symbol'], price['date']))
